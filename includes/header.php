...
require_once __DIR__ . '/action_icons.php';
require_once __DIR__ . '/session_timeout.php';

$is_admin_page = (strpos($_SERVER['PHP_SELF'] ?? '', '/admin/') !== false);
if ($is_admin_page && isset($_SESSION['user_id'])) {
    if (!checkAdminSessionTimeout()) {
        session_destroy();
        header('Location: ' . BASE_URL . '/auth/employee-login.php?session_expired=1');
        exit;
    }
}
?>
...
<body class="h-full">
    <?php if ($is_admin_page && isset($_SESSION['user_id']) && defined('SESSION_TIMEOUT_SECONDS')): ?>
    <!-- Session timeout warning (center of page, admin only) -->
    <div id="sessionTimeoutWarningModal" class="hidden fixed inset-0 z-[10000] flex items-center justify-center p-4" aria-modal="true" role="dialog" aria-labelledby="sessionTimeoutTitle">
        <div class="fixed inset-0 bg-black/60 dark:bg-black/70 backdrop-blur-sm" onclick="sessionTimeoutStayLoggedIn()"></div>
        <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-md w-full p-8 text-center border border-amber-200 dark:border-amber-800">
            <div class="mx-auto w-14 h-14 rounded-full bg-amber-100 dark:bg-amber-900/40 flex items-center justify-center mb-6">
                <svg class="w-8 h-8 text-amber-600 dark:text-amber-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
            </div>
            <h2 id="sessionTimeoutTitle" class="text-xl font-semibold text-gray-900 dark:text-white mb-2">Session expiring</h2>
            <p class="text-gray-600 dark:text-gray-400 mb-4">You will be logged out in <strong id="sessionTimeoutCountdown">0</strong> seconds due to inactivity.</p>
            <p class="text-sm text-gray-500 dark:text-gray-500 mb-6">Click below to stay logged in.</p>
            <button type="button" onclick="sessionTimeoutStayLoggedIn()" class="w-full px-6 py-3 bg-primary-600 hover:bg-primary-700 text-white font-medium rounded-lg focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800">
                Stay logged in
            </button>
        </div>
    </div>
    <script>
    (function() {
        var timeoutSeconds = <?php echo (int) SESSION_TIMEOUT_SECONDS; ?>;
        var warningBefore = <?php echo (int) (defined('SESSION_WARNING_BEFORE_SECONDS') ? SESSION_WARNING_BEFORE_SECONDS : 30); ?>;
        var heartbeatUrl = '<?php echo BASE_URL; ?>/admin/session_heartbeat.php';
        var countdownInterval = null;
        var warningShown = false;
        var lastHeartbeat = Date.now();
        var heartbeatInterval = 30000; // Send heartbeat every 30 seconds on activity

        function extendExpiry() {
            window.__sessionExpiresAt = Date.now() + timeoutSeconds * 1000;
        }
        ...
        function showWarning() {
            ...
            warningShown = true;
            var modal = document.getElementById('sessionTimeoutWarningModal');
            var cd = document.getElementById('sessionTimeoutCountdown');
            if (!modal || !cd) return;
            modal.classList.remove('hidden');
            function tick() {
                var remaining = Math.max(0, Math.ceil((window.__sessionExpiresAt - Date.now()) / 1000));
                cd.textContent = remaining;
                if (remaining <= 0) {
                    if (countdownInterval) clearInterval(countdownInterval);
                    window.location.href = '<?php echo BASE_URL; ?>/auth/employee-login.php?session_expired=1';
                }
            }
        ...
        window.sessionTimeoutStayLoggedIn = function() {
            fetch(heartbeatUrl, { credentials: 'same-origin' })
                .then(function(r) { return r.json(); })
                .then(function(data) {
                    if (data && data.ok) {
        ...
        extendExpiry();
        // Send heartbeat on any user activity to keep server session alive
        ...
        // Also send periodic heartbeat as long as page is active
        setInterval(function() {
            // If page is visible and user has been active recently, send heartbeat
            if (!document.hidden && (Date.now() - lastHeartbeat < heartbeatInterval * 2)) {
                sendHeartbeat();
            }
        }, heartbeatInterval);
        
        setInterval(function() {
            var remaining = (window.__sessionExpiresAt - Date.now()) / 1000;
            if (remaining <= warningBefore && remaining > 0 && !warningShown) {
                showWarning();
            } else if (remaining <= 0 && !warningShown) {
                window.location.href = '<?php echo BASE_URL; ?>/auth/employee-login.php?session_expired=1';
            }
        }, 500);
    })();
    ...
    <?php endif; ?>
    <!-- Full Page Loader -->
    <div id="pageLoader" class="fixed inset-0 z-[9999] bg-white dark:bg-gray-900 flex flex-col items-center justify-center transition-opacity duration-300 ease-out" style="display: flex !important; opacity: 1 !important;">
        <div class="flex flex-col items-center justify-center">
            <!-- Logo -->
            <div class="mb-8 animate-pulse">
                <img src="<?php echo BASE_URL; ?>/assets/img/alvion-logo-removebg.png" alt="Alvion Logo" class="h-20 md:h-24 w-auto">
            </div>
            
            <!-- Spinner -->
            <div class="relative w-16 h-16 mb-6">
                <div class="absolute inset-0 border-4 border-[#008080]/20 rounded-full"></div>
                <div class="absolute inset-0 border-4 border-transparent border-t-[#008080] rounded-full animate-spin"></div>
            </div>
            
            <!-- Loading Text -->
            <div class="text-center">
                <p class="text-lg font-semibold text-gray-900 dark:text-white mb-2" id="loaderText">Loading...</p>
                <div class="flex items-center justify-center space-x-1">
                    <div class="w-2 h-2 bg-[#008080] rounded-full animate-bounce" style="animation-delay: 0s;"></div>
                    <div class="w-2 h-2 bg-[#008080] rounded-full animate-bounce" style="animation-delay: 0.2s;"></div>
                    <div class="w-2 h-2 bg-[#008080] rounded-full animate-bounce" style="animation-delay: 0.4s;"></div>
                </div>
            </div>
            
            <!-- Progress Bar (Optional) -->
            <div class="w-64 h-1 bg-gray-200 dark:bg-gray-700 rounded-full mt-6 overflow-hidden">
                <div id="loaderProgress" class="h-full bg-gradient-to-r from-[#008080] to-[#0f766e] rounded-full transition-all duration-300" style="width: 0%"></div>
            </div>
        </div>
    </div>
    ...
    <div class="flex h-screen bg-gray-50 dark:bg-gray-900">
        ...
        $sidebar_map = [
            ...
            'doctor'      => __DIR__ . '/sidebars/sidebar_doctor.php',
            'attending'   => __DIR__ . '/sidebars/sidebar_doctor.php',
            'resident'    => __DIR__ . '/sidebars/sidebar_doctor.php',
            'consultant'  => __DIR__ . '/sidebars/sidebar_doctor.php',
        ];

        // Use doctor sidebar when in doctor portal path (for any role allowed there)
        $script_path = $_SERVER['PHP_SELF'] ?? '';
        if (strpos($script_path, '/doctor/') !== false && file_exists(__DIR__ . '/sidebars/sidebar_doctor.php')) {
            $sidebar_path = __DIR__ . '/sidebars/sidebar_doctor.php';
        } else {
        ...
        <div id="mainContent" class="flex-1 flex flex-col overflow-hidden transition-all duration-300 ease-in-out">
                        ...
                        <div class="relative ml-2">
                            ...
                            <button type="button" id="userMenuButton" class="flex items-center text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white p-1 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors focus:outline-none focus:ring-2 focus:ring-primary-500">
                                <div class="relative">
                                    ...
                                    <?php else: ?>
                                        <div class="w-10 h-10 bg-purple-500 rounded-full flex items-center justify-center text-white flex-shrink-0 font-semibold text-sm">
                                            <?php echo $user_initials; ?>
                                        </div>
                                    <?php endif; ?>
                                    <span class="absolute -bottom-1 -right-1 w-5 h-5 rounded-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 flex items-center justify-center shadow-sm">
                            ...
                            <div id="userDropdown" class="hidden absolute top-full right-0 mt-2 w-64 bg-white dark:bg-gray-800 rounded-lg shadow-2xl z-50 border border-gray-200 dark:border-gray-700 overflow-hidden">
                                ...
                                <div class="px-4 py-3 border-b border-gray-200 dark:border-gray-700">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <?php else: ?>
                                            <div class="w-12 h-12 bg-purple-500 rounded-full flex items-center justify-center text-white flex-shrink-0 font-semibold text-sm">
                                                <?php echo $user_initials; ?>
                                            </div>
                                        <?php endif; ?>
            ...
            <!-- Main Content Area -->
            <main class="relative flex-1 overflow-y-auto p-6 page-enter bg-gray-50 dark:bg-gray-900">
    <?php else: ?>
    <!-- Public Layout -->
    <main>
    <?php endif; ?>